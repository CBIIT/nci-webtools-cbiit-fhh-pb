...
export class LambdaWriteAnnotationsStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props: LambdaWriteAnnotationsStackProps) {
    super(scope, id, props);

    const tier = process.env.TIER || "dev";

    // Create IAM role for Lambda function
    const lambdaRole = new iam.Role(this, "WriteAnnotationsLambdaExecutionRole", {
    ...
    this.lambdaFunction = new lambda.Function(this, "WriteAnnotationsFunction", {
    ...
    const lambdaTags = createTags({ tier, resourceName: "lambda-write-annotations" });
    Object.entries(lambdaTags).forEach(([key, value]) => {
      cdk.Tags.of(this.lambdaFunction).add(key, value);
    });

    // Create CloudWatch alarms for monitoring
    const errorAlarm = new cloudwatch.Alarm(this, "WriteAnnotationsLambdaErrorAlarm", {
    ...
    const durationAlarm = new cloudwatch.Alarm(this, "WriteAnnotationsLambdaDurationAlarm", {
    ...
    // Outputs
    new cdk.CfnOutput(this, "WriteAnnotationsLambdaFunctionName", {
