...
def get_family(study_id, family_id, bucket_name=None):
    ...
    try:
        ...
        logger.debug(f"Successfully retrieved family data for study_id: {study_id}, family_id: {family_id}")
        return {"status": "success", "data": data}

    except boto3.client("s3").exceptions.NoSuchKey:
        error_msg = f"Family data not found for study_id: {study_id}, family_id: {family_id}"
