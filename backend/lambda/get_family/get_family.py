...
def get_family(family_id, bucket_name=None):
    ...
    try:
        ...
        logger.info(f"Successfully retrieved family data for family_id: {family_id}")
        return {"status": "success", "data": data}
        
    except boto3.client('s3').exceptions.NoSuchKey:
        error_msg = f"Family data not found for family_id: {family_id}"
        logger.warning(error_msg)
        return {"status": "not_found", "message": error_msg}
    except Exception as e:
        error_msg = f"Error retrieving family data for family_id {family_id}: {str(e)}"
