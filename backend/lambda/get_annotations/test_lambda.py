...
def test_get_annotations_success():
    ...
    s3.put_object(
        Bucket=bucket_name,
        Key=f"annotations/{study_id}/{family_id}.annotations.json",
        Body=test_data,
    )

    # Test the function
    result = get_annotations(study_id, family_id, bucket_name)
...
def test_get_annotations_no_bucket():
    ...
    result = get_annotations("test_study_001", "test_family")
...
def test_lambda_handler_success():
    ...
    event = {"pathParameters": {"study_id": study_id, "family_id": family_id}}
    context = MagicMock()
    ...
    with patch.object(lambda_module, "get_annotations") as mock_get:
        ...
        mock_get.assert_called_once_with(study_id, family_id)


def test_lambda_handler_missing_family_id():
    """Test lambda_handler with missing family_id."""
    event = {"pathParameters": {"study_id": "test_study_001"}}
    context = MagicMock()
...
def test_lambda_handler_not_found():
    ...
    event = {"pathParameters": {"study_id": "test_study_001", "family_id": "missing_family"}}
    context = MagicMock()
...
def test_lambda_handler_error():
    ...
    event = {"pathParameters": {"study_id": "test_study_001", "family_id": "error_family"}}
    context = MagicMock()
