import {
  ...
  find_children,
  find_all_partners,
  find_children_from_both_parents,
  find_all_parents_of_list,
  build_family_tree_with_ancestors,
  build_entire_family_tree,
  expand_one_generation_to_include_partners,
  expand_next_generation_to_include_all_children,
  determine_sex,
  find_orphaned_people,
  place_orphaned_people,
  find_in_tree,
  find_first_in_generation,
  find_last_in_generation,
  set_all_locations,
  find_parent_of_male_in_couple,
  find_parent_of_female_in_couple,
...
test("Test determine age by birthdate", () => {
  const proband = d.general?.proband;
...
test("Test that the FindChildren function works for the example proband", () => {
  ...
  const children = find_children(proband);
...
test("Test that we can find children of both parents without using the exception", () => {
  const children = find_children_from_both_parents(
    "10001-01-001",
    "10001-01-002"
  );
  ...
  const children_from_different_mom = find_children_from_both_parents(
    "10001-01-001",
    "10001-01-003"
  );
...
test("Test see if we can find all great-great-grandparents", () => {
  ...
  const grandparents_list = find_all_parents_of_list(parents_list);
  const great_grandparents_list = find_all_parents_of_list(grandparents_list);
  const great_great_grandparents_list = find_all_parents_of_list(
    great_grandparents_list
  );
...
test("Test expand_next_generation_to_include_all_children", () => {
  ...
  let next_generation = expand_next_generation_to_include_all_children(
    family_tree[0],
    family_tree[1]
  );
...
test("Test build_entire_family_tree", () => {
  const proband = d.general?.proband;
  const family_tree = build_entire_family_tree(proband);
  console.log(family_tree);
});
/*
